-- StudyNurse v0.4.1 NON-DESTRUCTIVE BUGFIX UPGRADE
-- Existing study_documents payload is not changed.

insert into public.study_revision_log(doc_key, payload, source_version, reason)
select d.doc_key,
       d.payload,
       coalesce(d.payload->>'version','legacy'),
       'upgrade-baseline-v0.4.1'
...
and not exists (
  ...
  where l.doc_key=d.doc_key
    and l.reason='upgrade-baseline-v0.4.1'
);
