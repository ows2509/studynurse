...
PREV=""
for x in "$ROOT/StudyNurse-v0.7.2" "$ROOT/StudyNurse-v0.7.1" "$ROOT/StudyNurse-v0.7.0"; do
...
for c in config.js config.dev.js; do
  if [[ -f "$PREV/$c" ]]; then
    cp -f "$PREV/$c" "$HERE/$c"
    sed -i 's/version:[[:space:]]*"[^"]*"/version: "0.7.3"/' "$HERE/$c"
  fi
done
rm -rf "$HERE/.git"; cp -a "$PREV/.git" "$HERE/.git"; cd "$HERE"
git fetch origin
./verify_version.sh
echo "Supabase SQL: supabase_upgrade_0.7.3.sql"
echo "git add -A"
echo "git commit -m 'StudyNurse v0.7.3'"
