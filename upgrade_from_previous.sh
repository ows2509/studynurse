...
for c in config.js config.dev.js;do [[ -f "$V/$c" ]]&&{ cp -f "$V/$c" "$R/$c";sed -i 's/version:[[:space:]]*"[^"]*"/version: "0.6.0"/' "$R/$c";};done
rm -rf "$R/.git";cp -a "$V/.git" "$R/.git";cd "$R";git fetch origin;./verify_version.sh
echo "Supabase SQL: supabase_upgrade_0.6.0.sql";echo "git add -A";echo "git commit -m 'StudyNurse v0.6.0'";echo "git rebase origin/main";echo "git push"
